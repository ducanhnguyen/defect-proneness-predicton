...
import com.defectpronenesspredicton.object.ITermRoot;
import com.defectpronenesspredicton.object.JDTRecord;
import com.defectpronenesspredicton.object.SingleTermRoot;
import com.defectpronenesspredicton.object.TermRoots;
	...
	public TermRoots analyzeTermsInBugDefectProneness() {
		...
		if (jdtCsvFile != null && jdtCsvFile.exists()) {
			...
			for (JDTRecord record : records)
				...
				for (String term : record.getLowercaseTerms(rankingMode)) {
			...
			termRoots.sortByFscore();

			// Improve the model
			// Add some terms
			System.out.println("Start adding new terms");
			int TOP_K = 10;
			for (int i = 0; i <= TOP_K - 2; i++)
				for (int j = i + 1; j <= TOP_K - 1; j++)
					for (int k = i + 2; k <= TOP_K; k++) {
